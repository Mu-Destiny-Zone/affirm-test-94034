...
export function TestAssignmentDialog({ test, open, onOpenChange, onAssignmentUpdated }: TestAssignmentDialogProps) {
  ...
  const handleAssign = async () => {
    ...
    // Prevent assigning draft or archived tests
    if (test.status === 'draft' || test.status === 'archived') {
      toast({
        title: 'Cannot Assign Test',
        description: test.status === 'draft' 
          ? 'Tests must be set to "Active" status before they can be assigned. Please change the test status first.'
          : 'Archived tests cannot be assigned. Please change the test status first.',
        variant: 'destructive'
      });
      return;
    }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
              ...
              <div className="flex gap-2">
                ...
                <div>
                  <p className="font-semibold text-yellow-800 dark:text-yellow-300">
                    {test.status === 'draft' ? 'Draft Test' : 'Archived Test'}
                  </p>
                  <p className="text-sm text-yellow-700 dark:text-yellow-400">
                    {test.status === 'draft' 
                      ? 'This test is in draft status and cannot be assigned. Change the test status to "Active" before assigning.'
                      : 'This test is archived and cannot be assigned. Change the test status to "Active" before assigning.'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-between pt-4 border-t">
            ...
            <Button onClick={handleAssign} disabled={loading || !newAssignment.assignee_id || test.status === 'draft' || test.status === 'archived'}>
