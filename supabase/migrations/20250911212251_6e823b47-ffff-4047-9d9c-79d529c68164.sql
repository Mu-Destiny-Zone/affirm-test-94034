...
BEGIN
    ...
    IF NEW.author_id = (
        CASE NEW.target_type
            WHEN 'bug' THEN (SELECT reporter_id FROM bug_reports WHERE id = NEW.target_id)
            WHEN 'suggestion' THEN (SELECT author_id FROM suggestions WHERE id = NEW.target_id)
            WHEN 'test' THEN (SELECT org_id FROM tests WHERE id = NEW.target_id) -- Will handle differently below
        END
    ) THEN
        RETURN NEW;
    END IF;
...
-- Enable realtime for notifications table (notifications already in publication)
